...
int main(void) 
{
    ...
    for (int i=0; i<R; i++) {
        for (int j=0; j<C; j++) {
            ...
            else if (board[i][j] == 'F') pos_F.push_back(make_pair(i,j));
        }
    }

    if (pos_J.X==0 || pos_J.X==R-1 || pos_J.Y==0 || pos_J.Y==C-1) {
        cout << 1;
        return 0;
    }
    ...
    for (int i=0; i<R; i++) {
        if (board[i][0] == '#') {} 
            ...
            if (dist[i][0] == 0) {
                if (dist_J[i][0] < exit_T) exit_T = dist_J[i][0];
            } else if (dist_J[i][0] < dist[i][0] && dist_J[i][0] < exit_T) exit_T = dist_J[i][0];
        }
        if (board[i][C-1] == '#') continue;
        else if (board[i][C-1]=='.' && dist_J[i][C-1]!=0) {
            if (dist[i][C-1] == 0) {
                if (dist_J[i][C-1] < exit_T) exit_T = dist_J[i][C-1];
            } else if (dist_J[i][C-1]<dist[i][C-1] && dist_J[i][C-1]<exit_T) exit_T = dist_J[i][C-1];
        }
    }
    for (int i=0; i<C; i++) {
        if (board[0][i] == '#') {}
            ...
            if (dist[0][i] == 0) {
                if (dist_J[0][i] < exit_T) exit_T = dist_J[0][i];
            } else if (dist_J[0][i]<dist[0][i] && dist_J[0][i]<exit_T) exit_T = dist_J[0][i];
        }
        if (board[R-1][i] == '#') continue;
        else if (board[R-1][i]=='.' && dist_J[R-1][i]!=0) {
            if (dist[R-1][i] == 0) {
                if (dist_J[R-1][i] < exit_T) exit_T = dist_J[R-1][i];
            } else if (dist_J[R-1][i]<dist[R-1][i] && dist_J[R-1][i]<exit_T) exit_T = dist_J[R-1][i];
        }
    }
