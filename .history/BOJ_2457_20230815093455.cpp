...
int main(void) {
	...
	while (compare_date(flowers[i][open], start)) {
		if (compare_close(flowers[i], flowers[picked])) picked = i;
		i++;
		if (i >= N) {
			in_garden = -1; 
			break;
	...
	while (compare_date(flowers[picked][close], end) && i<N) {
		while (compare_date(flowers[i][open], flowers[picked][close]) && i<N) i++;
		if (i<N) {
			picked = i-1;
			in_garden++;
		} else {
